...
Debug Tool: <https://github.com/Ariorh1337/phaser-debug-tool> (A utiliser sous Chrome)
...
### Fonts vs Bitmap Fonts

Phaser permet d’afficher du texte principalement de deux manières : avec des **fonts classiques (web fonts / TTF / OTF)** ou avec des **bitmap fonts**.

#### Fonts classiques

Les fonts classiques utilisent la classe `Phaser.GameObjects.Text`.  
Le texte est rendu dynamiquement par le navigateur via le canvas.

Avantages :
- Très simple à utiliser
- Aucun asset à préparer
- Texte facilement modifiable dynamiquement
- Support complet du style (taille, couleur, stroke, alignement, etc.)

Inconvénients :
- Le rendu est **plus coûteux** : le texte est rasterisé dans le canvas
- Chaque changement de texte peut déclencher un **re-render du canvas**
- Peut devenir **moins performant** si beaucoup de textes sont affichés ou mis à jour en permanence
...
Les bitmap fonts utilisent une **texture contenant tous les glyphes** de la police et un fichier de description en `.xml` pour Phaser.  
Chaque caractère devient simplement un **sprite affiché depuis un atlas**.

Avantages :
- **Beaucoup plus performant**
- Pas de rasterisation canvas
- Chaque caractère est juste un quad GPU
- Très efficace pour du texte fréquemment mis à jour

Inconvénients :
- Nécessite de **générer la font à l’avance**
- Jeu de caractères limité aux glyphes exportés
- Moins flexible pour le style

Cas d’usage typiques :
- score
- timer
- tout texte mis à jour à chaque frame

### Références
- Créer des bitmap fonts: <https://snowb.org/>
...
- Visualiser les courbes de easing des tweens: <https://easings.net/>
- Créer des sons: <https://sfxr.me/>
...
- Spawn rate, mouvement et spread shot doivent etre configurable via JSON (Ex: tirer aussi bien 3 dans un angle
  de 30 deg, que 60 bullets à 360 deg)
- Graphismes de votre choix

### Idées de features additionnelles
- Spawn enemies in waves/pattern
- Si vous voulez ajouter des boss/enemies, faites d'abord votre évaluation !
